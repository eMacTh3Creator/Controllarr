...
**Status:** v2.1.1 — bug-fix release. Fixes the root cause of torrents "randomly pausing themselves" (the shim was writing per-torrent connection caps into libtorrent's session queueing keys, so any library bigger than 15 torrents hit the auto-managed queue cap and got excess torrents silently paused). Per-torrent caps now go to `torrent_handle::set_max_connections` / `set_max_uploads` where they belong; session queueing is raised to effectively-unlimited (10k) by default. Adds a proper **Force Resume** action (right-click a torrent, or `POST /api/v2/torrents/setForceStart` with `value=true` for qBittorrent parity) that takes a torrent out of the auto-managed pool so nothing can silently re-pause it. And makes libtorrent queueing a real **Settings → Torrent queueing** toggle with configurable active-downloads / active-seeds / active-limit caps; when queueing is on, libtorrent auto-promotes the next queued torrent whenever a running one finishes, is paused, or is removed. Built on v2.1.0 (duplicate detection / force recheck / right-click menus / multi-select / port-cycle reconnect), v2.0.2 (category filter dropdown + menu-bar reliability), v2.0.1 (resume sidecars + periodic save), and v2.0.0 (peer-discovery toggles, connection-count limits, WebUI hardening, category-aware file moves, Torrents sort/filter, Settings redesign). See [Releases](https://github.com/eMacTh3Creator/Controllarr/releases) for a pre-built binary.
