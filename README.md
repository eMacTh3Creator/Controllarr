...
**Status:** v2.1.0 — feature release focused on qBittorrent-parity torrent management. Adds duplicate-torrent detection (ignore / merge-new-trackers / ask) so re-adds from Sonarr / Radarr no longer fail silently or pile up, a Force Recheck action that re-hashes on-disk pieces (skip re-downloading data you already have), right-click context menus on the Torrents table and Categories list (pause / resume / force recheck / reannounce / open in Finder / copy magnet / change category / remove), multi-select on the Torrents table for mass operations (mass pause, mass recheck, mass reannounce, mass delete), and a stronger port-cycle reconnect kick — `forceReannounceAll` now bypasses tracker announce-interval backoff and also hits DHT and LSD so peers learn the new listen port through every channel immediately. Built on v2.0.2 (category filter dropdown + menu-bar reliability), v2.0.1 (resume sidecars + periodic save), and v2.0.0 (peer-discovery toggles, connection-count limits, WebUI hardening, category-aware file moves, Torrents sort/filter, Settings redesign). See [Releases](https://github.com/eMacTh3Creator/Controllarr/releases) for a pre-built binary.
